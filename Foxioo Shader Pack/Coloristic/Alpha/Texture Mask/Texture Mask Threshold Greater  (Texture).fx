...
float2 Fun_RotationX(float2 In.texCoord)
{
    ...
    return _Points + mul(float2x2(_Cos, _Sin, -_Sin, _Cos), In.texCoord - _Points);
}
...
float4 ps_main(in PS_INPUT In) : COLOR0
{
    ...
    float2 _ScaleEx = float2(_ScaleX, _ScaleY) * _Scale;
            ...
            float4 _Render = tex2D(_Texture, UV);
                ...
                float _Result_Lum = dot(_Result.rgb, float3(0.2126, 0.7152, 0.0722)) * _Result.a;
                
                    if(_Result_Lum > (1.0 - _Mixing))
                    {
                        if(!_Overlay)
                            _Result.a = 0.0;
                        else
