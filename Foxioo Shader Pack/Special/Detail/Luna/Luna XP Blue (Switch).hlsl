...
static float4 _OverlayTop    = float4(0.0, 0.28, 0.84, 1.0);
static float4 _OverlayBottom = float4(0.0, 0.08, 0.57, 1.0);

static float3 _InnerLight     = float3(0.25, 0.59, 1.0);
static float3 _InnerShadow    = float3(0.0, 0.14, 0.63);
...
float3 Fun_Sharp(Texture2D _Texture, SamplerState _Sampler, float2 In, float2 _Off, float4 _Tint)
{
    ...
    float3 _Render = normalize(float3(_Emboss.x, _Emboss.y, 0.15));
