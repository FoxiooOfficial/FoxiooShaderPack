...
PS_OUTPUT ps_main( in PS_INPUT In )
{
    ...
    float4 _Result = _Blending_Mode ? Fun_Interpolation(In.texCoord, __Segments, S2D_Background, S2D_BackgroundSampler, _BlurScale) 
                                    : Fun_Interpolation(In.texCoord, __Segments, S2D_Image, S2D_ImageSampler, _BlurScale) * In.Tint;
...
PS_OUTPUT ps_main_pm( in PS_INPUT In ) 
{
    ...
    float4 _Result = _Blending_Mode ? Fun_Interpolation(In.texCoord, __Segments, S2D_Background, S2D_BackgroundSampler, _BlurScale) 
                                    : Demultiply(Fun_Interpolation(In.texCoord, __Segments, S2D_Image, S2D_ImageSampler, _BlurScale)) * In.Tint;
