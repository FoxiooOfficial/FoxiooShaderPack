...
float4 Fun_Kuwahara(float2 UV, Texture2D<float4> _Texture, SamplerState _Sampler, bool _Premultiplied, float4 _Tint)
{
    ...
    float2 _Offset = float2(fPixelWidth, fPixelHeight) * _Size;
        ...
        for (int _Y = -RADIUS; _Y <= RADIUS; _Y++)
        {
            for (int _X = -RADIUS; _X <= RADIUS; _X++)
            {
                ...
                float4 _Render = Demultiply(_Texture.Sample(_Sampler, UV + _Off) * _Tint, _Premultiplied);
...
float4 Main(in PS_INPUT In, bool _Premultiplied) : SV_TARGET
{
    float4 _Render_Texture = Demultiply(S2D_Image.Sample(S2D_ImageSampler, In.texCoord) * In.Tint, _Premultiplied);
        ...
        float4 _Result = Fun_Kuwahara(In.texCoord, S2D_Image, S2D_ImageSampler, _Premultiplied, In.Tint);
