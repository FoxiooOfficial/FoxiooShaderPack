...
float4 ps_main(in float2 In : TEXCOORD0, in float2 In_Background : TEXCOORD1) : COLOR0
{
    ...
    _Render_Background.a = _Render_Texture.a;
        ...
        int2 _Pixel = !_Blending_Mode ? float2((int)(In.x / fPixelWidth), (int)(In.y / fPixelHeight)) :
                                        float2((int)(In_Background.x / fPixelWidth), (int)(In_Background.y / fPixelHeight));

        float _PixelSizeFix = _PixelSize == 0.0 ? 0.0001 : _PixelSize;
        float2 _Size = float2(1.0 / fPixelWidth, 1.0 / fPixelHeight) / _PixelSizeFix;

        float2 _UV = !_Blending_Mode ?  float2(ceil(In.x * _Size.x) / _Size.x, ceil(In.y * _Size.y) / _Size.y) : 
                                        float2(ceil(In_Background.x * _Size.x) / _Size.x, ceil(In_Background.y * _Size.y) / _Size.y);
