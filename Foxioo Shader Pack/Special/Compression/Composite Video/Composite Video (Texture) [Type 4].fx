...
float4 ps_main(in PS_INPUT In) : COLOR0
{
    ...
    float2 _Pos = float2(floor(In.texCoord.x * SAMPLES), floor(In.texCoord.y * LINES + (_Time * FPS * 2.0)));
        ...
        if(_Distortion == 0.0) _UV = 0.0;
            ...
            // Luma
            float4 _Result = tex2D(S2D_Image, In.texCoord + _UV);
            float _Luma = Fun_RGB2YUV(_Result.rgb).x;

                _Result.rgb = Fun_Blur(S2D_Image, In.texCoord + _UV, 0.85);
                float3 _Render = Fun_RGB2YUV(_Result.rgb);
                float Y = _Render.x;

            // Chroma
            _Result.rgb = Fun_Blur(S2D_Image, In.texCoord + float2(0.0, 2.0 * (_Vert / LINES)) * Y + _UV, 4.0);
