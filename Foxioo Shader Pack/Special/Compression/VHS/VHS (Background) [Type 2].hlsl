...
float4 Main(in PS_INPUT In, bool _Premultiplied) : SV_TARGET
{
    ...
    float4 _Render_Background = S2D_Background.Sample(S2D_BackgroundSampler, UV);
       ...
       float4 _Result = _Render_Background;
        ...
        float3 _Render_Emboss = Fun_Sharp(S2D_Background, S2D_BackgroundSampler, UV, _EOffset);
        ...
        float _W = 0.0;
            ...
            for(i = 0; i < _Quality; i++)
            {
                ...
                float _Weight = sin((float(i + 1) / float(_Quality)) * M_PI);
                    ...
                    float3 _Render_ColorB = S2D_Background.Sample(S2D_BackgroundSampler, UV - _BOffset * _Size).rgb;
