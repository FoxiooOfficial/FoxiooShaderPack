...
float4 Main(in PS_INPUT In, bool _Premultiplied) : SV_TARGET
{
    ...
    float4 _Render_Background = S2D_Background.Sample(S2D_BackgroundSampler, In.bgCoord);
        ...
        float _Render_Texture_Lum = Fun_Luminance(S2D_Image.Sample(S2D_ImageSampler, In.texCoord + float2(fPixelWidth, fPixelHeight) * (_Lum * 0.1 - _LumBg * 0.0025)).rgb * In.Tint.rgb);
