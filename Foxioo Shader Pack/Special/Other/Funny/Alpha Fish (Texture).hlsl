...
float4 Main(in PS_INPUT In, bool _Premultiplied) : SV_TARGET
{
    ...
    float2 _In = In.texCoord;
        ...
        _Cirl *= lerp(0.95, 1.0, 1.0 - pow(In.texCoord.y - 0.55, 0.0));
            ...
            float _Depth = pow(abs(0.55 - In.texCoord.y), 3.0);
            ...
            float2 _Fish1In = In.texCoord;
                ...
                _Fish1In.y += 0.61;
                _Fish1In.x *= 1.04;
                float _Fish1 = ((_Fish1In.y + (pow(abs(_Fish1In.x), 0.14 - pow(_Fish1In.x, 3.0)) * (1.0 - _Fish1In.x * 1.2)) * 0.50) * 0.50 > 0.5) * (_Fish1In.x > 0.0) * (_Fish1In.y < 1.0) * (_Fish1In.x < 0.6);
