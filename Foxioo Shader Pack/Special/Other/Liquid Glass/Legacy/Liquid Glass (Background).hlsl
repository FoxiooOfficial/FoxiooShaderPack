...
float4 Main(in PS_INPUT In, bool _Premultiplied) : SV_TARGET
{
    ...
    float2 _UV = lerp(_UVB, _UVM, _Mask * _Mask * _Distortion);
        ...
        for(int i = 0; i < 36; i++)
        {
            float _Mul = ((uint)i % 2) ? 1.0 : -1.0;
            _Render_Background += S2D_Background.Sample(S2D_BackgroundSampler, _UV + Fun_Hash21(In.texCoord) * float(5.0 + i) * _Mul * float2(fPixelWidth, fPixelHeight));
        }
