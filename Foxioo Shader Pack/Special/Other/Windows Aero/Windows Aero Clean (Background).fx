...
float4 ps_main(float2 In : TEXCOORD0) : COLOR0
{
    ...
    float4 _Render_Background = tex2D(S2D_Background, In_Background) * _Render_Texture;
    ...
    float4 _Render = float4(lerp(_Render_Texture.rgb, _Render_Background.rgb + (_Outline.rgb + _Render_Texture.rgb * 0.1) * 0.25, _Mixing), _Render_Texture.a);
