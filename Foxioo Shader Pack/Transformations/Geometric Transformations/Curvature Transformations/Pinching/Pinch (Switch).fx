...
float2 Fun_Pinch(in float2 In.texCoord : TEXCOORD0)
{
    ...
    In.texCoord = lerp(_Rel, _UV, _Mixing) + _Center;

    return In.texCoord;
}

float2 Fun_RotationX(float2 In.texCoord: TEXCOORD)
{
    float2 _Points = float2(_PointX, _PointY);
    float2 _UV = In.texCoord;
    _RotX = _RotX * (3.14159265 / 180);

        _UV = _Points + mul(float2x2(cos(_RotX), sin(_RotX), -sin(_RotX), cos(_RotX)), _UV - _Points);
