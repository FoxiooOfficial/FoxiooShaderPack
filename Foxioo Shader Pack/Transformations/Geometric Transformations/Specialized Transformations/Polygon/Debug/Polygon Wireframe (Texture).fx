...
float4 ps_main(in float2 In : TEXCOORD0, in float3 Polygon : TEXCOORD1, in float  WReflected : TEXCOORD2) : COLOR0
{
    ...
    /* pixel shader */            float _Size = max(fwidth(_UV).x, fwidth(_UV).y);

            float _Wireframe =  smoothstep(_Size, 0.0, _UV.x) +
                                smoothstep(_Size, 0.0, 1.0 - _UV.x) +
                                smoothstep(_Size, 0.0, _UV.y) +
                                smoothstep(_Size, 0.0, 1.0 - _UV.y);

        float _Alpha = saturate(_Wireframe + smoothstep(_Size * 1.5, 0.0, abs(_UV.x + _UV.y - 1.0))) >= 0.25;

        float4 _Render_Texture = tex2D(S2D_Image, frac(_UV * _Scale * float2(_ScaleX, _ScaleY) + float2(_PosX, _PosY)));

    return _Render_Texture * _Alpha;
}
