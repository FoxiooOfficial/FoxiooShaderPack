...
PS_OUTPUT ps_main(in PS_INPUT In)
{
    ...
    float4 _Polygon = Fun_Polygon(In.texCoord);
        ...
        /* pixel shader */
        float4 _Render_Texture = S2D_Image.Sample(S2D_ImageSampler, frac(In.texCoord * _Scale * float2(_ScaleX, _ScaleY) + float2(_PosX, _PosY))) * In.Tint;
        _Render_Texture.a *= 1.0 - S2D_Image.Sample(S2D_ImageSampler, _UV).a * In.Tint.a;
...
PS_OUTPUT ps_main_pm(in PS_INPUT In)
{
    ...
    float4 _Polygon = Fun_Polygon(In.texCoord);
        ...
        /* pixel shader */
        float4 _Render_Texture = Demultiply(S2D_Image.Sample(S2D_ImageSampler, frac(In.texCoord * _Scale * float2(_ScaleX, _ScaleY) + float2(_PosX, _PosY)))) * In.Tint;
        _Render_Texture.a *= 1.0 - S2D_Image.Sample(S2D_ImageSampler, _UV).a * In.Tint.a;
