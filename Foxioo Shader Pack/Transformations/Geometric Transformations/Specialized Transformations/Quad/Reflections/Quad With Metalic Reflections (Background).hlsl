...
PS_OUTPUT ps_main( in PS_INPUT In )
{
    ...
    float _Luminance = dot(_Render_Texture.rgb, float3(0.299, 0.587, 0.114));
         ...
         _Render_Texture.rgb += S2D_Image.Sample(S2D_ImageSampler, _In.xy + (_Luminance_Background * 0.01) * _Mixing).rgb * In.Tint.rgb * _Mixing * _Luminance_Background * _Luminance;
...
PS_OUTPUT ps_main_pm( in PS_INPUT In )
{
    ...
    float _Luminance = dot(_Render_Texture.rgb, float3(0.299, 0.587, 0.114));
         ...
         _Render_Texture.rgb += S2D_Image.Sample(S2D_ImageSampler, _In.xy + (_Luminance_Background * 0.01) * _Mixing).rgb * _Mixing * In.Tint.rgb * _Luminance_Background * _Luminance;
