...
{  
    ...
    float4 _Render_Background = tex2D(S2D_Background, abs(frac((float2(_In.y, _In.x) + float2(_PosX, _PosY)) / 2.0) * 2.0 - 1.0));
    float _Luminance_Background = dot(_Render_Background.rgb, float3(0.299, 0.587, 0.114));

        _Render_Texture.rgb += (tex2D(S2D_Image, _In.xy / _Luminance_Background / 10.0).rgb + _Luminance_Background / 3.0) * _Mixing * _Luminance_Background;
